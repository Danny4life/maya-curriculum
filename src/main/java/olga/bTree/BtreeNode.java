...
class Btree{
    ...
    private BtreeNode searchRec(BtreeNode node, int key) {
        ...
        while (i < node.numKeys && key > node.keys[i]) {
                i++; // Find the first key greater than or equal to key
        }
        ...
        if(node.leaf){
            return null; // Reached leaf, key not found
        }
        // Recur to the appropriate child
        return searchRec(node.children[i], key); // Continue search
    }
    ...
    public static void main(String[] args) {
        ...
        tree.inOrder(); // Expected output: 5 6 7 10 12 17 20 30
